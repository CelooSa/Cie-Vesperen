...
const artistLog = async (req, res) => {
  try {
    const foundArtist = await artist.findOne({ email: req.body.email });
    if (!foundArtist) return res.status(404).json("Artist not found");
    ...
    if (!comparePassword)
      return res.status(400).json("Authentification failure !");
